...
namespace GameEngine.DataStructures
{
    ...
    public class BspTree
    {
        ...
        // --------------------------------------------------------------
        // Raycasting icin BSP destekli isin tarama
        // --------------------------------------------------------------
        // Sadece isinin gectigi BSP bolgelerindeki duvarlari toplar.
        // Ortalama O(log n + k); k = isinin gectigi bolgelerdeki duvar sayisi
        // En kotu  O(n) (isin tum bolme dogrularini kesiyorsa)
        // --------------------------------------------------------------
        public List<WallSegment> CollectAlongRay(float ox, float oy, float ex, float ey)
        {
            List<WallSegment> result = new List<WallSegment>();
            CollectRayRecursive(_root, ox, oy, ex, ey, result);
            return result;
        }

        private void CollectRayRecursive(BspNode node,
            float ox, float oy, float ex, float ey, List<WallSegment> result)
        {
            if (node == null) return;

            int sOrigin = ClassifyPoint(ox, oy, node.Partition);
            int sEnd = ClassifyPoint(ex, ey, node.Partition);

            // Bolme dogrusundaki duvari her zaman ekle (isin onu kesebilir)
            result.Add(node.Partition);

            // Isin iki tarafa da uzaniyorsa her iki alt agaci da gez
            if (sOrigin != sEnd)
            {
